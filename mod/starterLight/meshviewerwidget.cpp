...
void MeshViewerWidget::mouseMoveEvent( QMouseEvent* _event )
{
    ...
    if ( (_event->buttons() == (Qt::MiddleButton)) ||  (_event->buttons() == Qt::LeftButton && _event->modifiers() == Qt::ControlModifier))
    {
        float value_y = radius_ * dy * 3.0 / h;
        translate(Vec3f(0.0, 0.0, value_y));
    }
    else if ( (_event->buttons() == Qt::RightButton) || (_event->buttons() == Qt::LeftButton && _event->modifiers() == Qt::AltModifier) )
    {
